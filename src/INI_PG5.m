...
function [ini_theta] = INI_PG5(F)

k = 5; % k is the number of clusters
FF = F(:);
idx = kmeans(FF, k); % idx is the index.
% F1 
G = [FF idx];

% F5 are the samples that belong to the first population
F5 = G(G(:, 2)==5);
% F4 are the samples that belong to the second population
F4 = G(G(:, 2)==4);
% F3 are the samples that belong to the third population
F3 = G(G(:, 2)==3);
% F1 are the samples that belong to the fourth population
F1 = G(G(:, 2)==2);
% F2 are the samples that belong to the fifth population
F2 = G(G(:, 2)==1);
...
pi3 = length(F3)/length(FF);
pi4 = length(F4)/length(FF);
...
beta4 = mean(F4)/var4;
alpha4 = mean(F4)*beta4;

beta5 = mean(F5)/var5;
alpha5 = mean(F5)*beta5;
...
ini_theta = [pi1 alpha1 beta1 pi2 alpha2 beta2 ...
             pi3 alpha3 beta3 pi4 alpha4 beta4  alpha5 beta5];
