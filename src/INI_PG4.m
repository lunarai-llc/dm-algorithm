...
function [ini_theta] = INI_PG4(F)

k = 4; % k is the number of clusters
FF = F(:);
idx = kmeans(FF, k); % idx is the index.
% F1 
G = [FF idx];

% F4 are the samples that belong to the first population
F4 = G(G(:, 2)==4);
% F3 are the samples that belong to the second population
F3 = G(G(:, 2)==3);
% F1 are the samples that belong to the third population
F1 = G(G(:, 2)==2);
% F2 are the samples that belong to the fourth population
F2 = G(G(:, 2)==1);
...
ini_theta = [pi1 alpha1 beta1 pi2 alpha2 beta2 pi3 alpha3 beta3 alpha4 beta4];
