function out = mix_mc_identifyK_perMethodFast_contam(B, n, model, trueParams, varargin)
% MIX_MC_IDENTIFYK_PERMETHODFAST_CONTAM
% Monte Carlo over B datasets WITH CONTAMINATION (outliers).
%
% For EACH method (EM / HD / VNED), on each dataset:
...
%
% Contamination:
%   - Inject epsilon fraction of outliers after generating the clean mixture.
%   - Outliers ~ Uniform(a,b); integer counts by default ('uniform_int').
%
% Output fields:
%   out.khat.(EM|HD|VNED)               -> [B x 1] K̂ per method
%   out.sel_counts.(EM|HD|VNED){b}      -> selection split count struct (per method)
%   out.sel_frac_true.(EM|HD|VNED)      -> fraction of selection splits choosing true K
...
%   out.avg_when_correct.(method)       -> average over datasets where that method's K̂==trueK
%   out.prop_correct.(method)           -> % datasets where that method's K̂==trueK
%   out.settings                        -> echo of settings
%
% Requires:
%   mix_split_select_estimate.m  (version that supports ComputeHessian/UseParallel/ParallelAxis)
%
% Example:
% truth_pg.pi    = [0.3; 0.7];
% truth_pg.theta = [10 1; 1 2];   % PG: [a1 a2; b1 b2]
% out = mix_mc_identifyK_perMethodFast_contam( ...
%     200, 5000, 'pg', truth_pg, ...
%     'epsilon',0.20,'outlier_range',[15 25], ...   % 20% U(15,25) outliers (integer counts)
%     'methods',{'em','hd','vned'}, 'Kmax',5, ...
%     'n_sub', 1000, 'sel_C',1, 'sel_nStarts',1, 'sel_MaxIter',60, ...
%     'fit_C',5, 'fit_nStarts',2, 'fit_MaxIter',120, ...
...
ip.addParameter('epsilon', 0.40, @(x)isnumeric(x)&&isscalar(x)&&x>=0&&x<=1);                % fraction of outliers
ip.addParameter('outlier_range', [60 120], @(v)isnumeric(v)&&numel(v)==2&&v(1)<v(2));        % Uniform(a,b)
ip.addParameter('outlier_mode', 'uniform_int', @(s)ischar(s)&&ismember(lower(s),{'uniform_int','uniform_cont'}));
...
% 0) simulate full data (with contamination)
Yfull = generate_dataset(S.n, model, truth, S.epsilon, S.outlier_range, S.outlier_mode);
...
% =============================== Utils ================================
% ======================================================================
function Y = generate_dataset(n, model, truth, epsilon, outlier_range, outlier_mode)
% Generate clean mixture, then inject an epsilon-fraction of outliers.
% Outliers ~ Uniform(a,b); integer by default (fast), or continuous then rounded.

if nargin < 4 || isempty(epsilon),       epsilon = 0;            end
if nargin < 5 || isempty(outlier_range), outlier_range = [25 35]; end
if nargin < 6 || isempty(outlier_mode),  outlier_mode  = 'uniform_int'; end
...
% ----- inject outliers (epsilon fraction) -----
m = round(epsilon * n);
if m > 0
    idx = randperm(n, m);
    a = outlier_range(1); b = outlier_range(2);
    switch lower(outlier_mode)
        case 'uniform_int'   % fast integer counts
            Y(idx) = randi([ceil(a) floor(b)], m, 1);
        otherwise            % 'uniform_cont' -> continuous then round
            Y(idx) = round(a + (b-a)*rand(m,1));
    end
end
...
% Return the first averaged-params struct {K, pi, theta} regardless of cell/struct.
...
% Compute NaN-robust average across a cell array of structs {K, pi, theta}.
